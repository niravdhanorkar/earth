...
console.log('GIO', GIO)
  ...
  linesOrange = new THREE.Group();

  // scene.add(linesGrey, linesWhite, linesRed, linesBlue);
  mainGroup.add(linesYellow, linesWhite, linesRed, linesBlue, linesOrange);
  scene.add(mainGroup);
  ...
  Array(5)
    ...
    .fill()
    .forEach((val, index) => {
      const line = createLine(101, 'orange');
      line.rotation.y = (Math.PI / 2.5) * 1.5 * index;
      line.position.x = 10 * index;
      line.position.y = -2;
      linesOrange.add(line);
    });
  // FOR BLUE LINES
  Array(2)
    .fill()
  ...
  lineGroups = [linesBlue, linesYellow, linesWhite, linesRed, linesOrange];
